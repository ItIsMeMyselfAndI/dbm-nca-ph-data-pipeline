    ...
    try:
        ...
        for record in event['Records']:
            try:
                ...
                logger.error(f"Failed to process queued record: {e}")

        elapsed = str(timedelta(seconds=time.time() - prev_time)).split(":")
        logger.debug(f"Processing & Loading complete: "
                     f"{elapsed[0]}h {elapsed[1]}m {elapsed[2]}s")

    except Exception as e:
        logger.critical(f"Processing/Loading job failed: {e}", exc_info=True)
        return {"statusCode": 400,
                "body": "Processing/Loading job failed."}
